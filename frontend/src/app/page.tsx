    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9FAFB] p-4 font-sans">
        <form onSubmit={handleLogin} className="p-8 bg-white rounded-2xl border border-slate-200 shadow-sm w-full max-w-sm">
          ...
          <p className="text-center text-slate-500 mb-8 text-sm">Sign in with agent credentials</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition-colors flex items-center gap-1 shadow-sm"
            >
            ...
            <div key={session.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2">
                  <button
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 rounded-lg text-xs font-medium text-slate-600 transition-colors pointer-events-none w-full justify-center"
                  >
                  ...
                  {getDisplayStatus(session) === 'Active' && (
                    <>
                      <button
                        onClick={(e) => { e.stopPropagation(); copyLink(session.invite_token) }}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 hover:border-slate-300 shadow-sm rounded-lg text-xs font-medium text-slate-700 transition-colors w-full justify-center"
                      >
                        {copied === session.invite_token ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3 text-slate-400" />}
                        {copied === session.invite_token ? 'Copied' : 'Invite'}
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); joinSession(session.id) }}
                        className="flex items-center gap-1.5 px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white shadow-sm rounded-lg text-xs font-medium transition-colors w-full justify-center"
                      >
                        Join
                      </button>
                    </>
                  ...
                  {getDisplayStatus(session) === 'Active' && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 border border-red-200 hover:bg-red-100 rounded-lg text-xs font-semibold text-red-700 transition-colors w-full justify-center"
                    >
